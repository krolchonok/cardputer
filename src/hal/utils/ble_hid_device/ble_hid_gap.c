...
static void handle_ble_device_result(struct ble_scan_result_evt_param *scan_rst)
{
    ...
    uint8_t *appearance_d =
        esp_ble_resolve_adv_data_by_type(scan_rst->ble_adv, scan_rst->adv_data_len + scan_rst->scan_rsp_len,
...
esp_err_t esp_hid_ble_gap_adv_init(uint16_t appearance, const char *device_name)
{
    ...
    // Use Just Works pairing for better compatibility with Windows/iOS
