...
class AppWifiMenu::CategorySelectorMenu : public SmoothSelectorMenu {
public:
    struct OptionInfo_t {
        int appId = -1;
        std::string name;
...
AppWifiMenu::AppWifiMenu()
{
    setAppInfo().name     = "WiFi";
...
void AppWifiMenu::onOpen()
{
    ...
    if (_scan_app_id >= 0) {
        options.push_back({_scan_app_id, "Scan", _scan_app_icon});
    }
    _menu->init(options);
...
void AppWifiMenu::resolve_app_ids()
{
    _scan_app_id   = -1;
    _scan_app_icon = nullptr;
    ...
    for (auto& app_raw : installed_apps) {
        ...
        if (app->getAppInfo().name == "Scan") {
            _scan_app_id = app->getId();
            _scan_app_icon = static_cast<AppIcon_t*>(app->getAppInfo().userData);
        }
    }
}
